...
# ### 하드웨어
# **하드웨어**는 데스크탑, 노트북, 테블릿, 스마트폰 등과 같이 컴퓨터를 구성하고 있는 기계 장치의 총칭으로 크게 본체와 주변 장치로 나뉜다. 본체는 중앙 처리 장치(CPU, Central Processing Unit)와 주기억 장치(Main memory), 주변 장치는 입·출력 장치, 보조 기억 장치 등으로 구분된다. 
#   - **중앙처리장치**: 컴퓨터 시스템을 통제하고 프로그램의 연산을 실행·처리하는 가장 핵심적인 컴퓨터의 제어 장치, 혹은 그 기능을 내장한 칩이다.(위키백과)
#   - **주기억장치**: 컴퓨터에서 수치·명령·자료 등을 기억하는 컴퓨터 하드웨어 장치이다.(위키백과)
...
# 파이썬은 웹 어플리케이션의 개발 언어로도 많이 사용되고 있으며, 데이터 분석에 장점을 가진 스크립트 언어이며, 기계학습 등 인공지능의 개발에도 많이 사용되고 있는 등 웹/앱에서 인공지능까지 인기가 급상승하며 주목받는 언어이다. 
# 
...
# - ...
# 
# 여기에서는 가장 기본적인 파이썬 IDLE, 개발 환경으로 인기가 많은 Jupyter Notebook과 Google Colab에 대해서 소개한다.

# ### 구글 Colab

# **구글 코랩**은 브라우저 내에서 Python 스크립트를 작성하고 실행할 수 있도록 구글에서 제공하는 에디터이다. 주피터 노트북기반으로 제공하므로 주피터 노트북을 사용해본 개발자라면 구글 코랩도 쉽게 익힐 수 있다. 
# 구글 코랩은 구글 클라우드 서비스를 기반으로 무료로 리소스(RAM, Disk)를 제공하며 구글 아이디만 있으면 사용할 수 있다. 개인의 PC 보다 접근성이 좋으며 링크하나로 다른 사용자들과의 공유와 협업도 용이하다는 장점이 있다.
# 

# #### 코랩 실행하기
# 
# 웹브라우저에서 https://colab.research.google.com/ 링크로 코랩에 접속해보자. 코랩에 접속하면 코랩에 대한 간단한 소개글 페이지가 열린다. 코랩 사용법을 정리해둔 문서이니 필요하다면 읽어보길 바란다. 코랩 노트에서는 아래 그림과 같이, 코드에 대한 설명, 결과 내용, 그림, 링크 등을 넣는 텍스트 셀, 코드를 넣는 코드 셀, 그리고 코드를 실행하면 바로 결과까지 볼 수 있다.
# 
...
# 셀을 실행하는 방법은 셀 앞에 플레이 버튼처럼 생긴 실행버튼을 눌러도 되고, 단축키를 사용해도 된다. 
# 
...
# 
# 코드셀은 코랩 메뉴 아래에 '+코드'를 클릭하거나, 추가하고자 하는 셀 아래 중간쯤에 마우스를 움직이면 '+코드', '+텍스트' 버튼이 나타나는데 여기에서 '+코드'를 클릭하여 추가할 수 있다.
...
# #### 파일명 변경하기
# 파일의 이름을 변경하기 위해서는 코랩 마크 옆에 Untitled0.ipynb 부분을 클릭하여 변경할 수 있다. 코랩이나 주피터 노트북에서 작성한 코드는 확장자가 .ipynb를 갖는다. 코랩은 구글 드라이브와 연동되며 여러분이 저장한 노트는 구글 드라이브의 **Colab Notebooks** 폴더에 자동 저장된다.

# <div align="center"><img src="https://haesunbyun.github.io/Basic-Computing/_images/ch01/colab_2.png" style="width:700px;"></div>
